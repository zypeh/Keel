...
Token scan(SrcScanner *scanner) {
        ...
        case VERTICALTAB:
                ...
                while (scanner->pos <= len || src[scanner->pos] != '\0') {
                        if (src[scanner->pos] == SPACE || src[scanner->pos] == TAB ||
                            src[scanner->pos] == FORMFEED || src[scanner->pos] == VERTICALTAB ||
                            (src[scanner->pos] == CARRIAGERETURN && src[scanner->pos + 1] != LINEFEED)) {
                                next(scanner, 1);
                                continue;
                        }
                        break;
        ...
        case MINUS:
                if (src[scanner->pos + 1] == MINUS) {
                        next(scanner, 1);
                        while (scanner->pos <= len && src[scanner->pos] != '\0') {
                                if (scanner->pos + 1 <= len || src[scanner->pos + 1] != Newline ||
                                    src[scanner->pos + 1] != '\0') {
                                        next(scanner, 1);
